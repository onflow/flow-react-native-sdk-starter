...
// Path to the fcl-react-native package (symlinked via file: reference)
...
config.resolver.extraNodeModules = {
  ...
  // React Query - MUST use test app's version
  "@tanstack/react-query": path.resolve(
    __dirname,
    "node_modules/@tanstack/react-query"
  ),

  // Viem - MUST use test app's version (peer dependency of SDK packages)
  ...
  "@onflow/react-native-sdk": path.join(fclJsRoot, "packages/react-native-sdk"),
  "@onflow/react-core": path.join(fclJsRoot, "packages/react-core"),
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  // Force React and peer dependencies to always resolve from test app's node_modules
  if (
    moduleName === "react" ||
    ...
    const modulePath = path.resolve(__dirname, "node_modules", basePath);
    try {
      const packageJson = require(path.join(modulePath, "package.json"));
      let mainFile;
      if (moduleName === "react/jsx-runtime") {
        mainFile = "jsx-runtime.js";
      } else {
        mainFile = packageJson["react-native"] || packageJson.main || "index.js";
      }
      return {
        type: "sourceFile",
        filePath: path.join(modulePath, mainFile),
      };
    } catch (e) {
  ...
  if (moduleName.startsWith("@onflow/")) {
    ...
    try {
      ...
      // For react-native-sdk and react-core, ALWAYS use built dist files
      // This ensures proper React resolution from the bundled code
      if (packageName === "react-native-sdk" || packageName === "react-core") {
        const distFile = packageJson.module || packageJson.main || "dist/index.module.js";
        const distPath = path.join(packagePath, distFile);
        return {
          type: "sourceFile",
          filePath: distPath,
        };
      ...
      // For other packages, if package.json has a "source" field, resolve to source files
...
// Block React and peer dependencies from being resolved from fcl-js node_modules to prevent duplicates
config.resolver.blockList = [
  new RegExp(`${fclJsRoot.replace(/[/\\]/g, "[/\\\\]")}/node_modules/react/.*`),
  new RegExp(`${fclJsRoot.replace(/[/\\]/g, "[/\\\\]")}/node_modules/react-dom/.*`),
  new RegExp(
    `${fclJsRoot.replace(/[/\\]/g, "[/\\\\]")}/node_modules/react-native/.*`
  ),
  new RegExp(
    `${fclJsRoot.replace(/[/\\]/g, "[/\\\\]")}/node_modules/@tanstack/react-query/.*`
  ),
  new RegExp(
    `${fclJsRoot.replace(/[/\\]/g, "[/\\\\]")}/node_modules/viem/.*`
  ),
];
